                ...
                },
                 {
                     ...
                     formatter: function (value, row, index) {
                         ...
                         if (row.Email == "") {
                             ...
                             if ($('#hdnusertype').val() == 3 && row.UserType== 1)
                             {
                     ...
                     type: 'search',
                     sortable: false,
                     formatter: operatePasswordFormatter,
                     events: operatePasswordEvents
                 }, {
                     field: 'UserType',
                     title: 'Role',
                     checkbox: false,
...
function operatePasswordFormatter(value, row, index) {
    return [
        '<button id="view" class="btn btn-link" title="View Password">',
            '<i class="fa fa-eye"></i>',
        '</button>'
    ].join('');
}
...
window.operateEvents = {
    'click #edit': function (e, value, row, index) {
        ...
        $('#industry option[value="' + row.IndustryId + '"]').prop("selected", true);
    },
...
window.operatePasswordEvents = {
    'click #view': function (e, value, row, index) {
        $.ajax({
            ...
            type: "POST",
            //contentType: "application/json; charset=utf-8",
            url: $_BaseUrl + "/Admin/ManageUser/GetPassword",
            data: { id: row.UserId },
            success: function (data) {
                if (data.status == true) {
                    BootstrapDialog.show({
                        title: 'Password',
                        message: 'Password for ' + row.FirstName + ' is:</br></br> ' + data.password,
                        buttons: [{
                            label: 'Ok',
                            cssClass: 'btn-danger',
                            action: function (dialogItself) {
